...
public class EjemploSockets2 {
	...
	public static void main(String[] args) {
		...
		final VentanaCliente vc = new VentanaCliente( "A" );
		vc.setVisible( true );
		(new Thread() {
			@Override
			public void run() {
		...
		final VentanaCliente vc2 = new VentanaCliente( "B" );
		...
		try {Thread.sleep(2000); } catch (InterruptedException e) {} 
		final VentanaCliente vc3 = new VentanaCliente( "C" );
	...
	public static class VentanaServidor extends JFrame {
		...
		public void lanzaServidor() {
			...
			try(ServerSocket serverSocket = new ServerSocket( PUERTO )) {
				...
				while (!finComunicacion) {
					try {
						final Socket socket = serverSocket.accept(); // Se queda esperando a una conexión con timeout
						// *VARIOS CLIENTES*
